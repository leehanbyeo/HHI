$(document).ready(function(){
    ...
    // #pcnav에 마우스를 올리면
        // #pcnav(this)의 높이가 340px(80+260)이 되도록 애니메이트
    ...
    $("header").after("<div id='dummy'></div>"); //html의 순수성 유지 위함
    function setHeader(){
    ...
    // .mmain을 눌렀을 때
        ...
        // 그렇지 않다면
            ...
            // 지금누른그것에게 .aactive.를 주기
        // 열기가 끝나면
        // 지금누른그것을 제외한 나머지 .mmain 안에 있는 .msub를 닫기
    $(".mmain").click(function(){
        ...
        $(this).siblings(".mmain").children(".msub").delay(300).slideUp(300);
    });
    ...
    // #linkbtn이 눌렸을 때
        // #linklist에서 value가 무엇인지 알아보고
        // 만약 그 값이 ""이 아니냐 - true, alert으로 확인
        // 만약 그 값이 null이 아니냐 - false
        // 만약 그 값이 undefined이 아니냐 - false
    ...
    // urlset이라는 아파트에서 page라는 이름을 가진 값이 몇층 몇호에 있는지 수색
    // 1. 우리 아파트가 몇층이지?
    // 2. 각 층별로 수색.
    ...
    // <쓰기set 단계>
    // #depth2 안쪽에 새로운 a들을 만들어 내는 단계
    // li의 개수만큼 반복
        // #depth2 안쪽에 새로운 a들을 만들기(href랑 text 넣어서)
    for(i=0; i<lilen; i++){
    ...
    // #depth1_mark안에 내용을 넣어주기
    $("#depth1_mark").text($("#depth1_cont>a").eq(mainnum).text());
    
    // #depth2_mark 내용 넣어주기
    // #depth2_cont안에 들어있는 a중에 mainnum번째의 글자내용을 알아내서
    // #depth2_mark안에 내용을 넣어주기
    ...
    //#depth1_cont, #depth2_cont에서 마우스를 치웠을 때
        // 그 자신이 사라진다.
...
//        var attrlen = 2;
        ...
        for(i=0; i<attrlen; i++){
            var key = kkubddegi[i].name;
            var value = kkubddegi[i].value;
            $(this).next(".newtable").children("table").attr(key,value);
        }
        $(this).remove();
    });
    
    // 여러 .tabbtn중에 하나를 클릭했을 때
        // 방금클릭한그것이 전체 .tabbtn중에서 몇 번째인가? => nth
        // 모든 .tabcont를 전부 숨겨준 다음에
        // .tabcont중에서 nth번째만 다시 보여준다.
        // 모든 .tabbtn으로부터 ""라는 클래스를 뺏는다.
        // .tabbtn중에서 nth번째에게는 "tactive"라는 클래스를 준다.
        // └---------------------this
    ...
    // "    "라는 리스트들 li태그 안쪽에 파란박스, 내용이 들어갈 박스 만들어주기
